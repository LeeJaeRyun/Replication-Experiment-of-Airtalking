...
`reports/final/finalization_manifest.json`은 1~3번, `reports/final/04_이전_현재_컴퓨터_실험_결과_비교_manifest.json`은 4번의 입력·출력 SHA-256과 생성 정보를 기록합니다. `reports/final`이 정본이며, `studies/*/reports/*Easy_Final_KR.docx`는 호환 경로를 위해 정본 DOCX 바이트를 그대로 복제한 파일입니다.

```powershell
.\.venv\Scripts\python.exe tools\build_easy_study_reports.py
```

내용과 실험 증거는 그대로 두고 표지·타이포그래피·표·그림 출처 등 DOCX 프레젠테이션만 7개 경로 전체에 다시 적용하려면 다음 명령을 사용합니다. 이 명령은 04 비교 보고서를 포함한 정본 4개와 호환본 3개를 모두 다시 만들고 DOCX hash manifest도 갱신합니다.

```powershell
.\.venv\Scripts\python.exe tools\refresh_all_docx_reports.py
```
